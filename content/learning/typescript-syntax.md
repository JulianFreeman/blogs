...
数组：`number[]`、`string[]` 等。等价于 `Array<number>{:ts}` 等。
...
给变量指定类型：`let name: string = "Alice";{:ts}`，但是大部分时候不需要手动指定。
...
> [!warning] 别名只是别名，与它所代指的类型别无二致。因此两个同一类型的别名也完全相同。
...
> [!tip] 其实 `boolean` 就是两个字面量值联合的别名：`true | false`。
...
> [!warning] 但是这只是一个针对编译器的声明，并不影响运行时，所以只有当你十分确定的时候再用。
...
> [!warning] 泛型不要滥用：能不用就不用；能不限制就不限制；能简单点就简单点。
...
> [!warning] 因为函数实现对外不可见，所以其上的重载函数至少有两个才有意义。
...
> [!warning] 同样的，函数重载能不用就不用，优先考虑联合类型。
